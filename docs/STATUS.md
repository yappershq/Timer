...
| HUD | ✅ DONE | HTML center panel, `h:mm:ss.mmm`. **Perf-color tint** (speed goes green while airborne off a perf bhop, read from the active mode's `IsPerfing`) + **compact panel** (`hud compact` per-player preference toggle, cs2kz IsCompactPanel) now done. Particle MHUD is addon-gated: cs2kz needs MultiAddonManager + shipped particle assets and falls back to this HTML HUD without them — which is exactly what we render. |
| Tip | ✅ DONE | Broadcast cycling with configurable interval (kz_tip_interval convar) + Fisher-Yates shuffle (no repeats within a cycle). Tip content is file-driven through the LocalizerManager JSON — the chosen mechanism, equivalent to cs2kz's KV tip files (per-culture, hot-editable), so no separate loader is needed. |
...
| Goto / Fov / Measure / Pistol / Noclip | ✅ DONE | Goto (timer-block), Fov, Pistol (strip-enforce + **team-awareness done** — transient m_iTeamNum switch around GiveNamedItem so either team renders any pistol, cs2kz UpdatePistol), Noclip (respawn safeguard), Measure (eye aim-trace + `clear` subcommand). |
